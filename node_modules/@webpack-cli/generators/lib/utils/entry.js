...
async function entry(self, multiEntries, autoGenerateDefaults = false) {
    ...
    if (multiEntries) {
        ...
        const multipleEntriesAnswer = await webpack_scaffold_1.InputValidate(self, 'multipleEntries', 'What do you want to name your bundles? (separated by comma)', validate_1.default, 'pageOne, pageTwo', autoGenerateDefaults);
        ...
        for (let i = 0; i < entryIdentifiers.length; i++) {
            ...
            const entryResult = await webpack_scaffold_1.InputValidate(self, `${entryProp}`, `What is the location of "${entryProp}"?`, validate_1.default, `src/${entryProp}`, autoGenerateDefaults);
            const entry = fixEntry(entryResult[entryProp]);
            webpackEntryPoint[entryProp] = entry;
        }
        return webpackEntryPoint;
    }
    const singleEntryResult = await webpack_scaffold_1.Input(self, 'singularEntry', 'Which will be your application entry point?', 'src/index', autoGenerateDefaults);
